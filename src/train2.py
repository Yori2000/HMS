...
def main(cfg : DictConfig):
    ...
    trainset, collate_fn        = get_dataset(cfg.dir.input, 'train', cfg.dataset)
    validset, _                 = get_dataset(cfg.dir.input, 'valid', cfg.dataset) 
            ...
            if current_step % cfg.step.valid == 0:
                with torch.no_grad():
                    ...
                    msg = "Valid\tEpoch : {}, Step : {}, valid Loss : {}"\
                            .format(epoch, current_step, valid_loss_avg)
                    logger.info(msg)

                    if cfg.analysis:
                        acc_table.log_wandb(current_step)
                        wandb.log({"valid loss avg":valid_loss_avg, "valid loss std": valid_loss_std}, step=current_step)
            # batch end---------------------------------------------------------------        
